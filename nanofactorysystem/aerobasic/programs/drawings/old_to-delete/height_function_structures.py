...
class HeightFunctions:
    ...
    def sinusoidal(period: float, height: float, angle_deg: float = 0,
        ...
        period: Gitterperiode (z.B. in µm)
        height: Gesamthöhe (z0 bis z0 + height)
    ...
    def sinusoidal(period: float, height: float, angle_deg: float = 0,
        ...
        """
        Sinusoidal grating: z = z0 + (h/2) * sin(2π/L * s + φ)
        where s(x,y) = x*cos(θ) + y*sin(θ)
        """
        theta = np.deg2rad(angle_deg)
    ...
    def binary_grating(period: float, height: float, duty_cycle: float,
        ...
        angle_deg: Rotation des gratings
        z0: Offset der Höhe
        height: Höhe des Gratingplateaus
        """
        theta = np.deg2rad(angle_deg)
