...
class Distance_Test(DrawableObject):
    ...
    def distances(self):
        if self._distances is None:
            self._distances = np.round(np.geomspace(self.min_dist, self.max_dist, self.n_lines))
    ...
    def iterate_layers(self, coordinate_system: CoordinateSystem) -> Iterator[DrawableAeroBasicProgram]:
        ...
        if self.draw_axis.lower() == "x":
            ...
            center_points = [self.center.Y - np.sum(self._distances) / 2]
            for i in range(len(self._distances)):
                center_points.append(center_points[-1] + self._distances[i])
            line_start = self.center.X - self.length / 2
        elif self.draw_axis.lower() == "y":
            line_program = YLines
            # center_points -> X, because print direction is y-axis
            center_points = [self.center.X - np.sum(self._distances) / 2]
            for i in range(len(self._distances)):
                center_points.append(center_points[-1] + self._distances[i])
