...
def binary_testprint(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
    ...
    if path is None:
        ...
        assert (path, Path)
        path = Path(mkdir(os.path.join(path, "angle_test_with_hatching"), clean=False))
    ...
    with Experiment(
            ...
            structure_size=structure_size*1.3,  # ToDo change fov to structure size and add fov to real
            margin=margin,
            padding=padding,
        ...
        for i in range(len(angles)):
            experiment.add_structure(
                ...
                name=f"binary_angle_{angles[i]}_hatchingAngle_{angles[i]}",
                axes="ABZ",
