...
def binary_testprint(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
    ...
    if objective == "Zeiss 20x":
        ...
        movement_axis = ["ABZ", "XYZ"]
        parameterset = {
            "hatch size": [0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5],  # [0.1, 0.2, 0.5],  # hatch size
            "slice size": [0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5],  # [0.1, 0.2, 0.5],  # slice size/ layer height
            "power": 0.5,
            "velocity": 5_000,
        }
    ...
    grid_size = (len(parameterset["hatch size"]), len(parameterset["slice size"]))  # number of repetitions, number of structures
    with Experiment(
            path=path,
            user=user,
            objective=objective,
        ...
        for i in range(len(parameterset["hatch size"])):
            for j in range(len(parameterset["slice size"])):
                experiment.add_structure(
                    ...
                    name=f"binary_s{parameterset["slice size"][j]}_h_{parameterset["hatch size"][i]}_p{parameterset["power"]}_v{parameterset["velocity"]}",
                    axes="ABZ",
