...
def binary_testprint(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
    ...
    if path is None:
        ...
        assert (path, Path)
        path = Path(mkdir(os.path.join(path, "grid_resolution_test"), clean=False))
    ...
    structure_size = 140.0
    grid_resolutions = [10_000, 5_000, 2_000, 1_000, 500, 200, 100, 50, 25, 10]
    grid_size = (1, len(grid_resolutions))
    with Experiment(
            path=path,
            user=user,
            objective=objective,
        ...
        for i in range(len(grid_resolutions)):
            experiment.add_structure(
                structure_type=StructureType.STITCHING,
                name=f"binary_{grid_resolutions[i]}",
                axes="ABZ",
