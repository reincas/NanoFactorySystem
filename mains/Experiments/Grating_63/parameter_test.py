...
def binary_testprint(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
    ...
    if path is None:
        ...
        assert (path, Path)
        path = Path(mkdir(os.path.join(path, "parameter_test"), clean=False))
    ...
    if objective == "Zeiss 20x":
        ...
        movement_axis = ["ABZ", "XYZ"]
        parameterset = {
            "hatch size": [0.2, 0.2, 0.2, 0.25, 0.25, 0.3],  # hatch size
            "slice size": [0.2, 0.2, 0.2, 0.35, 0.35, 0.5],  # slice size/ layer height
            "power": [0.2, 0.2, 0.5, 0.5, 0.9, 0.9],
            "velocity": [2_000, 2_000, 5_000, 5_000, 10_000, 10_000],
        }
    ...
    grid_size = (len(parameterset["hatch size"]), 2)  # number of repetitions, number of structures
    with Experiment(
            path=path,
            user=user,
            objective=objective,
        ...
        for i in range(len(parameterset["hatch size"])):
            experiment.add_structure(
                ...
                name=f"binary_s{parameterset["slice size"][i]}_h_{parameterset["hatch size"][i]}_p{parameterset["power"][i]}_v{parameterset["velocity"][i]}",
                axes="ABZ",
                ...
                structure=BinaryGrating(
                    ...
                    fov_size=(fov, fov),
                    usable_fov_fraction=0.75,
                    grid_resolution=10000)
            )
            experiment.add_structure(
                structure_type=StructureType.STITCHING,
                name=f"blazed_s{parameterset["slice size"][i]}_h_{parameterset["hatch size"][i]}_p{parameterset["power"][i]}_v{parameterset["velocity"][i]}",
                axes="ABZ",
                power=parameterset['power'][i],
