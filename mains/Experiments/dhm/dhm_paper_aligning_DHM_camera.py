...
def dhm_paper(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
              objective="Zeiss 63x", user="Hannes", repeat=12, dhm_usage=True):
    """
    ...
    if path is None:
        ...
        path = Path(mkdir(f".output/dhm_paper/FINAL_finalYES_print_{datetime.datetime.now():%Y%m%d}_{objective}", clean=False))
    else:
        assert (path, Path)
        path = Path(mkdir(os.path.join(path, "Test_centralising_structure"), clean=False))
    ...
    grid_size = (2, 2)  # number of repetitions, number of structures
    central_point_1 = Point3D(50, 50, -2)
    central_point_2 = Point3D(-50, -50, -2)
    ...
    with Experiment(
        ...
        experiment.add_structure(
            ...
            name=f"stair_{movement_axis[0]}_{objective}_50_50",
        ...
        experiment.add_structure(
            structure_type=StructureType.NORMAL,
            name=f"DOE_{movement_axis[0]}_{objective}_50_50",
            axes=movement_axis[0],
            power=parameterset["power"],
            structure=DOEstep(
                central_point_1,
        ...
        experiment.add_structure(
            structure_type=StructureType.NORMAL,
            name=f"stair_{movement_axis[0]}_{objective}_-50_-50",
            axes=movement_axis[0],
            power=parameterset["power"],
            structure=Stair(
                central_point_2,
        ...
        experiment.add_structure(
            structure_type=StructureType.NORMAL,
            name=f"DOE_{movement_axis[0]}_{objective}_-50_-50",
            axes=movement_axis[0],
            power=parameterset["power"],
            structure=DOEstep(
                central_point_2,
