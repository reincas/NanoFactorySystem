...
def print_file(absolute_center: Point2D, resin_dimension: list, ask_continue_box=False, path=None,
    ...
    if path is None:
        ...
        assert (path, Path)
        path = Path(mkdir(os.path.join(path, "ifov_lens"), clean=False))
    ...
    if objective == "Zeiss 20x":
        ...
        parameterset = {
            ...
            "slice size": [0.1, 0.15, 0.2, 0.25, 0.3],  # slice size/ layer height
    ...
    grid_size = (2, len(parameterset["slice size"]))
    ...
    with Experiment(
        ...
        for j in range(2):
            for i in range(len(parameterset["slice size"])):
